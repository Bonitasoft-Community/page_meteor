...
public class MeteorRobotUserActivity extends MeteorRobot {
    ...
    protected MeteorRobotUserActivity(final APIAccessor apiAccessor) {
        super(apiAccessor);
    }
            ...
            if (pendingTask != null) { // assign the task to the
                ...
                // update variable
                    ...
                    processAPI.updateActivityDataInstance(variableName, pendingTask.getId(),
                                mMeteorUser.mVariables.get(variableName) == null ? null
                                        : mMeteorUser.mVariables.get(variableName).toString());
                }
                // add documents
                    for (final meteorDocument toolHatProcessDefinitionDocument : mMeteorUser.mListDocuments) {
                    processAPI.attachDocument(pendingTask.getId(), toolHatProcessDefinitionDocument.mDocumentName, "TheFileName", "application/pdf",
                            toolHatProcessDefinitionDocument.mContent.toByteArray());
                }
