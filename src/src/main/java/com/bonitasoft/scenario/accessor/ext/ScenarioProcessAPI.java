...
public class ScenarioProcessAPI {
	...
	static public ProcessDefinition deployProcess(Accessor accessor, Map<String, Serializable> parameters) throws Exception {
		...
		if (!accessor.getDefaultProcessAPI()
				.searchProcessDeploymentInfos(
						new SearchOptionsBuilder(0, Integer.MAX_VALUE).filter(ProcessDeploymentInfoSearchDescriptor.NAME, businessArchive.getProcessDefinition().getName()).filter(ProcessDeploymentInfoSearchDescriptor.VERSION, businessArchive.getProcessDefinition().getVersion()).done())
				.getResult().isEmpty()) {
		...
		if (connectors != null) {
			...
			for (Map<Serializable, Serializable> connector : connectors) {
				...
				// only BONITASOFT SUBSCRIPTION
				// accessor.getDefaultProcessAPI().setConnectorImplementation(processDefinition.getId(),
				// (String)connector.get(Constants.NAME),
				// (String)connector.get(Constants.VERSION),
				// (byte[])connector.get(Constants.RESOURCE_NAME));
			}
		}
		...
		if (parametersResource != null) {
			...
			// only BONITASOFT SUBSCRIPTION
			// accessor.getDefaultProcessAPI().importParameters(processDefinition.getId(),
			// parametersResource);
		}

		// Add the auto generated forms by default for the process instantiation
		// form and human tasks in case it is
		if (defaultFormMapping == null || defaultFormMapping) {
			...
			formMappings.addAll(
					accessor.getDefaultProcessAPI().searchFormMappings(new SearchOptionsBuilder(0, Integer.MAX_VALUE).filter(FormMappingSearchDescriptor.PROCESS_DEFINITION_ID, processDefinition.getId()).filter(FormMappingSearchDescriptor.TYPE, FormMappingType.PROCESS_OVERVIEW).done()).getResult());
			formMappings.addAll(
					accessor.getDefaultProcessAPI().searchFormMappings(new SearchOptionsBuilder(0, Integer.MAX_VALUE).filter(FormMappingSearchDescriptor.PROCESS_DEFINITION_ID, processDefinition.getId()).filter(FormMappingSearchDescriptor.TYPE, FormMappingType.PROCESS_START).done()).getResult());
			...
			for (FormMapping formMapping : formMappings) {
				if (formMapping.getURL() == null) {
					...
					if (pages != null && !pages.isEmpty()) {
						...
						// only BONITASOFT SUBSCRIPTION
						// accessor.getDefaultProcessAPI().updateFormMapping(formMapping.getId(),
						// null, pages.get(0).getId());
					}
				}
			}
		}
	...
	static public ProcessInstance startProcess(Accessor accessor, Map<String, Serializable> parameters) throws Exception {
		...
		// Extract the initial variables if any and complement the operations
		// and context
		...
		// Extract the initial documents if any and complement the operations
		// and context
	...
	static public DataAccessorManager caseContext(Accessor accessor, Map<String, Serializable> parameters) throws Exception {
		...
		// ------- Load the context based on the information provided (provided
		// IDs are always active but task and case may be archived or active)
		...
		// ------- Retrieve and return all the variables and documents according
		// to the gathered context
