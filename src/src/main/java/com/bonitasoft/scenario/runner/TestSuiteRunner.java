...
public class TestSuiteRunner extends Runner {
	...
	public TestSuiteRunner(TestSuiteRunContext testSuiteRunContext, List<RunListener> runListeners) throws Exception {
		...
		// TODO: add a runListener to log the result of a test each time a test is over
	}
...
//			String testSuiteLocation = testSuiteRunContext.getMainContent() + File.separator;
...
//					testSuiteGSContent = testSuiteGSContent.replaceAll("PUT_UP", extractResourceFileContent(testSuiteFolder + File.separator + "putUp.groovy"));
//					testSuiteGSContent = testSuiteGSContent.replaceAll("BEFORE_EACH_TEST", extractResourceFileContent(testSuiteFolder + File.separator + "beforeEachTest.groovy"));
//					testSuiteGSContent = testSuiteGSContent.replaceAll("AFTER_EACH_TEST", extractResourceFileContent(testSuiteFolder + File.separator + "afterEachTest.groovy"));
//					testSuiteGSContent = testSuiteGSContent.replaceAll("TEAR_DOWN", extractResourceFileContent(testSuiteFolder + File.separator + "tearDown.groovy"));
					...
					// Launch the GS with a suitable context for the Test Suite to run over BOS API
//					testSuiteResult = (Map<String, Throwable>)groovyEvaluation(testSuiteGSContent, testSuiteRunContext.getDependencies());
//					testSuiteFolder.getAbsolutePath()

					// Log the result
					// TODO: use the listener pattern as done for the RunListener instead of logging all tests at the end
...
////		TestSuiteRunner testRunner = new TestSuiteRunner(new TestSuiteRunContext("example" + File.separator + "testSuite"));
//		TestSuiteRunner testRunner = new TestSuiteRunner(new TestSuiteRunContext("example" + File.separator + "testSuite", testSuiteNames));
//		testRunner.run();
//	}
}
